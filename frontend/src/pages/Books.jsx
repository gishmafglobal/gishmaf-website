...
const stripePromise = loadStripe(process.env.VITE_STRIPE_PUBLIC_KEY || "YOUR_STRIPE_PUBLISHABLE_KEY");

// Fake reviews (frontend display only)
const FAKE_REVIEWS = {
  book1: [
    { email: "alice@gmail.com", rating: 5, comment: "Absolutely loved this book!" },
    { email: "john@yahoo.com", rating: 5, comment: "Incredible story!" },
...
export default function Books() {
  ...
  const handlePurchase = async (bookId) => {
    ...
    try {
      ...
      try {
        ...
        console.error("[HANDLE PURCHASE] Failed to parse JSON:", e);
        alert("Purchase failed: invalid response from server. Check console for details.");
        setLoadingBook(null);
        return;
      }

      console.log("[HANDLE PURCHASE] Parsed JSON:", data);

      if (data.sessionId) {
        console.log("[HANDLE PURCHASE] Redirecting to Stripe checkout with sessionId:", data.sessionId);
        const stripe = await stripePromise;
        const { error } = await stripe.redirectToCheckout({ sessionId: data.sessionId });
        if (error) console.error("[HANDLE PURCHASE] Stripe redirect error:", error);
      } else {
        console.error("[HANDLE PURCHASE] SessionId not created. Backend error:", data.error);
        alert(`Purchase failed: checkout session not created. Backend message: ${data.error || "No error info"}`);
      }
    } catch (err) {
      console.error("[HANDLE PURCHASE ERROR]", err);
  ...
  return (
    <div style={{ backgroundColor: "#f4f6f9", minHeight: "100vh", padding: "60px 20px", fontFamily: "Segoe UI, sans-serif", color: "#111" }}>
      <h1 style={{ textAlign: "center", fontSize: "42px", fontWeight: "700", marginBottom: "50px" }}>📚 Our Books</h1>

      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: "40px", maxWidth: "1200px", margin: "0 auto" }}>
        {books.map((book) => (
          <div key={book.id} style={{ background: "#ffffff", borderRadius: "20px", padding: "25px", boxShadow: "0 10px 30px rgba(0,0,0,0.1)" }}>
            <img src={book.image} alt={book.title} style={{ width: "100%", height: "300px", objectFit: "cover", borderRadius: "15px", marginBottom: "20px" }} />
            ...
            {(reviews[book.id] || []).map((r, index) => (
              <div key={index} style={{ backgroundColor: "#f9fafb", padding: "15px", borderRadius: "12px", marginBottom: "12px", border: "1px solid #e5e7eb" }}>
                ...
                <div style={{ color: "#f59e0b", fontWeight: "600", marginBottom: "6px" }}>⭐ {r.rating}</div>
                <p style={{ fontSize: "14px", margin: 0 }}>{r.comment}</p>
              </div>
            ))}
          </div>
        ))}
