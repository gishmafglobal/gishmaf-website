...
const stripePromise = STRIPE_PUBLIC_KEY ? loadStripe(STRIPE_PUBLIC_KEY) : null;
if (!STRIPE_PUBLIC_KEY) console.error("⚠️ Stripe public key missing!");

const FAKE_REVIEWS = {
  book1: [
    { email: "alice@gmail.com", rating: 5, comment: "Loved this book!" },
    { email: "john@yahoo.com", rating: 5, comment: "Incredible story!" },
  ],
  book2: [
    { email: "mike@yandex.com", rating: 5, comment: "Changed my perspective!" },
...
export default function Books() {
  ...
  const handlePurchase = async (bookId) => {
    ...
    if (!stripePromise) {
      alert("Stripe public key missing");
      return;
    }
    if (!API_URL) {
    ...
    try {
      console.log("[PURCHASE] Starting:", bookId);
      ...
      if (data.sessionId) {
        ...
        alert(`Purchase failed: ${data.error || "No sessionId returned"}`);
      }
    } catch (err) {
